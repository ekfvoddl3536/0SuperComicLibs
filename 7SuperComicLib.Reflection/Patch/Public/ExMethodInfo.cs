...
namespace SuperComicLib.Reflection.Patch
{
    public class ExMethodInfo : IDisposable
    {
        ...
        public virtual bool GenerateCode(ILGenerator il, MethodBase original, ParameterInfo[] parameters, int offset, bool hasReturn, bool hasArgBuffer)
        {
            ...
            if (idx < len)
            {
                ...
                for (; idx < len; idx++)
                {
                    ...
                    if (param == null)
                        continue;
                    if (toTypes[idx].IsByRef)
                        il.Emit(OpCodes.Ldarga, (short)(param.Position + offset));
                    else
                        il.Emit(OpCodes.Ldarg, (short)(param.Position + offset));
                }
            }
            ...
            return patching.ReturnType == Helper.BOOL_T;
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                ClsArray.DisposeAll(ref toNames);
                patching = null;
