...
namespace SuperComicLib.Collections
{
    ...
    public class IndexLinkedList<T> : ICollection<T>, IReadOnlyCollection<T>, IEnumerable<T>, IValueIterable<T>, ILinkedListSlim_Internal<T>
    {
        ...
        {
            m_list = new Node[CMath.Clampi(capacity, 4, 0x7FEF_FFFF)];
            m_free_idx = -1;
        }
        ...
        public void RemoveAt(NodeIndex<T> node)
        {
            ...
            target_.next = -1; // flag empty slot
        }
        ...
        /// <summary>
        /// Find item using default comparer
        /// </summary>
        /// <returns>if <paramref name="value"/> cannot found, result is <see cref="int">-1</see></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public NodeIndex<T> Find(T value) => Find(value, EqualityComparer<T>.Default);

        /// <summary>
        /// Find item
        /// </summary>
        /// <returns>if <paramref name="value"/> cannot found, result is <see cref="int">-1</see></returns>
        public NodeIndex<T> Find(T value, EqualityComparer<T> comparer)
        {
            Node[] list = m_list;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Contains(T value) => Find(value).m_owner != null;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool Contains(T value, EqualityComparer<T> comparer) => Find(value, comparer).m_owner != null;
        #endregion
