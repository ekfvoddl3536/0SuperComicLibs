...
namespace SuperComicLib.LowLevel
{
    public unsafe static class NativeClass
    {
        ...
        public static T GetValue<T>(byte[] bytes) where T : unmanaged
        {
            fixed (byte* ptr = bytes)
                return *(T*)ptr;
        }

        public static T GetValue_s<T>(byte[] bytes) where T : unmanaged
        {
        ...
        public static byte[] ReadMemory<T>(ref T obj, uint count)
        {
            ...
            {
                fixed (byte* ptr = res)
                    memcpblk.Invoke(original.ToPointer(), ptr, count);
            });
            return res;
        }
